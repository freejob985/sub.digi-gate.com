...
class MilestonePaymentController extends Controller
{
    ...
    {
        if (Gate::allows('project_payment_history_index')) {
            $sort_search = null;
    ...
    public function check_for_earning_badge($user_id){
        $badges = Badge::where('type','earning_badge')->where('role_id', 2)->orderBy('value', 'desc')->get();
    ...
    public function check_for_spent_badge($user_id){
        $badges = Badge::where('type','spent_badge')->where('role_id', 3)->orderBy('value', 'desc')->get();
        foreach ($badges as $key => $badge) {
    ...
    {
        if (Gate::allows('milestone_payment_details')) {
    ...
    public function milestone_request_message(Request $request)
    {
        ...
        $role = Role::where('id',Session::get('role_id'))->first();
        if ($role->name == 'Client') {
