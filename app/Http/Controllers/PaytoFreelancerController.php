...
class PaytoFreelancerController extends Controller
{
    ...
    {
        if (Gate::allows('freelancer_payment_index')) {
            $sort_search = null;
    ...
    public function pay(Request $request)
    {
        ...
        if ($request->hasFile('photo')) {
            $pay_to_freelancer->reciept = $request->file('photo')->store('uploads/payment_reciept_for_freelancer');
        }
        ...
        if ($pay_to_freelancer->save()) {
            ...
            flash(__('Payment has been done successfully'))->success();
    ...
    public function cancel_request($id){
        ...
        flash(__('Payment has been cancelled'))->success();
        return redirect()->route('withdraw_request.index');
    }
    ...
    {
        if (Gate::allows('withdraw_request_index')) {
    ...
    public function send_withdrawal_request_store(Request $request)
    {
        if($request->amount <= Auth::user()->profile->balance && $request->amount >= \App\Models\SystemConfiguration::where('type', 'min_withdraw_amount')->first()->value){
            ...
            flash(__('Withdrawal request has been sent successfully'))->success();
