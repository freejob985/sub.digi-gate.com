                ...
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('support-tickets.update',$support_ticket->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control aiz-selectpicker" name="assigned_user_id" data-toggle="select2" data-placeholder="{{translate('Choose ...')}}" required>
                                @foreach ($employees as $key => $employee)
                                    <option value="{{ $employee->user->id }}" @if($employee->user->id == $support_ticket->assigned_user_id) selected @endif>{{ $employee->user->name }}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group mb-3">
                        <label for="commission_type">{{translate('Details')}}</label>
